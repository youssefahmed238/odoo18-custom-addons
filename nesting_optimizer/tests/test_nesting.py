...
def test_glass_factory_small_batch_detailed():
    ...
    sheet_cost = 25.00  # Example cost per sheet
    total_cost = sheets_used * sheet_cost
    cost_per_piece = total_cost / items_placed if items_placed > 0 else 0

    print(f"\n💰 COST ANALYSIS (Example):")
    print(f"   Sheet cost: €{sheet_cost:.2f} per sheet")
    print(f"   Total sheet cost: €{total_cost:.2f}")
