...
class MedicalPolicy(models.Model):
    ...
    apply_commission = fields.Boolean(string='Apply Contract Commission', default=True)
    ...
    commission_line_ids = fields.One2many('commission.line', 'medical_policy_id', store=True)
            ...
            if record.apply_commission and contract_line:
                ...
                if not record.commission_line_ids:
                    self.env['commission.line'].create({
                        ...
                        'total_amount_p': record.total_amount_p,
                    })
