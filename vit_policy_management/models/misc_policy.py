...
class MiscPolicy(models.Model):
    ...
    category = fields.Many2one(
        'policy.category',
        string="Category",
        default=lambda self: self._default_misc_category(), )

    misc_sequences = fields.Char(string="Sequences", readonly=True)
    ...
    product = fields.Many2one('policy.product', string="Product", domain=[('category_id', '=', 'MISC')])
    ...
    policy_risks_ids = fields.One2many('policy.risks', 'misc_policy_number', domain=[('state', '=', 'approved')])

    # risks_ids = fields.One2many('risks.line', 'misc_risks_id')
    # risks_policy_risks_premium_summary_ids = fields.One2many('risks.line', 'misc_risks_id')
    # risks_policy_premium_summary_ids = fields.One2many('risks.line', 'misc_risks_id')

    instalment_ids = fields.One2many('instalment.line', 'misc_policy_id')

    policy_premium_summary_charges_ids = fields.One2many('insurance.policy.premium.summary', 'misc_policy_id')
    ...
    def _default_misc_category(self):
