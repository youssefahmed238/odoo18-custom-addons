...
class MarinePolicy(models.Model):
    ...
    _sequence_code = "marine.policy.seq"
    _sequence_field = "marine_sequences"
    ...
    category = fields.Many2one(
        'policy.category',
        string="Category",
        default=lambda self: self._default_marine_category(), )

    marine_sequences = fields.Char(string="Sequences", readonly=True)
    ...
    product = fields.Many2one('policy.product', string="Product", domain=[('category_id', '=', 'Marine')])
    ...
    def create(self, vals):
        ...
        return super(MarinePolicy, self).create(vals)

    def _default_marine_category(self):
