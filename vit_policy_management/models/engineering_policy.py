...
class EngineeringPolicy(models.Model):
    ...
    name = fields.Char(string="Policy engineering", readonly=True)
    ...
    risks_ids = fields.One2many('risks.line', 'end_risks_id')
    risks_policy_risks_premium_summary_ids = fields.One2many('risks.line', 'end_risks_id')
    risks_policy_premium_summary_ids = fields.One2many('risks.line', 'end_risks_id')

    @api.model
    def create(self, vals):
        name = self.env['ir.sequence'].next_by_code('engineering.policy.seq')
        vals.update({
            'name': name,
